...
async def on_voice_channel_connect(member, channel):
    ...
    if channel.id in lobby_channels:
        ...
        if len(queue_dict[channel.id]) >= 4 and len(channel.members) >= 4:
            list_of_players = queue_dict[channel.id][0:4]
            ...
            queue_dict[channel.id] = queue_dict[channel.id][4:]
            ...
            assert len(lobby_channel.members) == 4, "Members less than 10 in lobby"
            ...
            if not_eligible_members:
                ...
                schedule.every().day.at(delete_category_at) \
                    .do(await delete_cat_roles(red, blue, captain.id, role, red_channel.category))
                return


async def delete_cat_roles(red, blue, captain_id, role, category):
    print('inside delete_cat_roles')
    match_id = category.name
    await role.delete()
    for channel in enumerate(category.channels):
...
#         print('inside on_voice_channel_alone for loop')
#         if i.type.name == 'text':
#             continue
...
#             await category_to_be_deleted.delete()
#             print('deleted category and removed roles')
#             break
#         else:
#             return
